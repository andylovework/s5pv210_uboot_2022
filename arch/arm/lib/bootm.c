...
{
	smp_kick_all_cpus();
	dcache_disable();	/* flush cache before swtiching to EL2 禁止处理器的缓存和内存管理单元 */
}
#endif

__weak void board_prep_linux(bootm_headers_t *images) { }

/* Subcommand: PREP 1.分配一块内存，把扁平设备树二级制文件复制过去 2.修改扁平设备树二进制文件*/
	...
	if (!fake) {
		...
		if ((IH_ARCH_DEFAULT == IH_ARCH_ARM64) &&
			...
			armv8_switch_to_el2(0, (u64)gd->bd->bi_arch_number,
					    (u64)images->ft_addr, 0,
					    (u64)images->ep,
					    ES_TO_AARCH32); /* 在异常级别1执行内核 */
		else
			armv8_switch_to_el2((u64)images->ft_addr, 0, 0, 0,
					    images->ep,
					    ES_TO_AARCH64); /* 在异常级别2执行内核 */
#endif
	}
#else
		   ...
		   bootm_headers_t *images)
{
	...
	boot_prep_linux(images);
	boot_jump_linux(images, flag); /* 跳转到内核入口 */
	return 0;
}
