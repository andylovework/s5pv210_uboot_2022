		    ...
		    int boot_progress)
{
	...
	if (states & BOOTM_STATE_START)
		ret = bootm_start(cmdtp, flag, argc, argv); /* 初始化全局变量bootm_headers_t images */

	if (!ret && (states & BOOTM_STATE_FINDOS))
		ret = bootm_find_os(cmdtp, flag, argc, argv); /* 把内核镜像从存储设备读到内存 */

	if (!ret && (states & BOOTM_STATE_FINDOTHER))
		ret = bootm_find_other(cmdtp, flag, argc, argv); /* 对于其他信息，对于ARM64通常是扁平设备树二进制文件，传递硬件信息给内核 */

	/* Load the OS */
	if (!ret && (states & BOOTM_STATE_LOADOS)) {
		iflag = bootm_disable_interrupts();
		ret = bootm_load_os(images, 0); /* 把内核加载到正确的位置，如果内核是被压缩过，需要解压 */
	...
	if (ret)
		return ret;
	boot_fn = bootm_os_get_boot_func(images->os.os); /* 根据操作系统类型在数组boot_os中查找引导函数 */
